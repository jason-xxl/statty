...
def stat_moagent(my_date):
    ...
    stat_plan=Stat_plan(plan_name='daily-moagent-error-page-umniah')
    ...
    stat_plan.add_log_source(r'\\192.168.1.40\umniah_moagent_logs\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
    stat_plan.add_log_source(r'\\192.168.1.41\moagent_logs_umniah\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    
    stat_plan.add_log_source(r'\\192.168.1.40\umniah_moagent_logs\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
    stat_plan.add_log_source(r'\\192.168.1.41\moagent_logs_umniah\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
        my_date,r'\\192.168.1.40\umniah_moagent_logs\PageError.log.%(date)s-%(hour)s', \
        timezone_offset_to_sg=config.timezone_offset_umniah,date_format='%Y-%m-%d'))
    stat_plan.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
        my_date,r'\\192.168.1.40\umniah_moagent_logs\PageError.log.%(date)s-%(hour)s', \
        timezone_offset_to_sg=config.timezone_offset_umniah,date_format='%Y-%m-%d'))
    stat_plan.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
        my_date,r'\\192.168.1.40\umniah_moagent_logs\PageErrorInfo.log.%(date)s-%(hour)s', \
        timezone_offset_to_sg=config.timezone_offset_umniah,date_format='%Y-%m-%d'))
    stat_plan.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
        my_date,r'\\192.168.1.40\umniah_moagent_logs\PageErrorInfo.log.%(date)s-%(hour)s', \
        timezone_offset_to_sg=config.timezone_offset_umniah,date_format='%Y-%m-%d'))
