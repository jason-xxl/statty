...
def stat_moagent(my_date):
    ...
    stat_plan=Stat_plan(plan_name='daily-moagent-error-page-telk-armor')
    ...
    stat_sql_url_error_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   ...
                                   group_by={'by_type':r' - ([^:]+: [^\t]+)\turl=', \
                                             ...
                                             'daily':r'(^\d+ \w+)'}, \
                                   db_name='data_url_pattern_telk_armor', \
    ...
    stat_plan.add_log_source(r'\\192.168.1.37\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.36\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.60\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.68\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))



    stat_plan.add_log_source(r'\\192.168.1.37\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.36\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
    
    stat_plan.add_log_source(r'\\192.168.1.60\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.68\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
    ...
    stat_plan.add_log_source(r'\\192.168.100.28\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.100.28\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.100.30\logs_moagent\PageError.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.100.30\logs_moagent\PageErrorInfo.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
