...
def stat_chatroom_king_invite(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    from telkomsel_mt.dbo.award_record with(nolock) where type ='chatroom'
    ...
    db='telkomsel_invitation_status'
    key='invite_chatroom_king_sub_unique'
    sql=r'''
    SELECT 
    sum([IsValid]) as sub_chatroom_invite

    FROM [telkomsel_invitation_status].[dbo].[invitation_statistics] with(nolock)
