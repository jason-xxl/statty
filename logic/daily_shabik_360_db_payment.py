...
def stat_billing(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    #exec('from user_id_filter.user_id_filter_shabik_360_'+current_date.replace('-','_')+' import user_list_dict')
    ...
    values=helper_sql_server.fetch_rows(config.conn_stc_billing,sql)
    ...
    for item_name,money_sum in values.iteritems():
        ...
        helper_mysql.put_raw_data(oem_name,stat_category,key,item_name+'_'+date_today,money_sum,table_name='raw_data_shabik_360')
