...
def is_correct_date(line):
    ...
    if not time:
        return False
    match=current_date in helper_regex.time_add(time+':00',config.timezone_offset_umniah*1.0/24)
    return match
...
def stat(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    stat_plan.add_raw_content_source(helper_file.get_http_content_with_cookie( \
            r'''http://angel.morange.com/rrd/img/5/download?start=%s&end=%s'''
...
if __name__=='__main__':
    ...
    for i in range(config.day_to_update_stat,0,-1):
        my_date=time.time()+3600*config.timezone_offset_umniah-3600*24*i
        stat(my_date)
