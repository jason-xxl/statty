...
def stat_user_generated_content(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.photos_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2" \
         % (start_time,end_time)
    ...
    sql='''select count(id) as msg,count(distinct user_id) as uv,count(id)/count(distinct user_id) as avg from mozone.dbo.moblog_msgs with(nolock)
         where id>=( select min(item_id) from mozone_tag.dbo.moblog_msgs_tags with(nolock) where partner_id=2 and
         createdon>='%s' and createdon<'%s' )
         and id in (
         select item_id from mozone_tag.dbo.moblog_msgs_tags with(nolock) where partner_id=2 and
         createdon>='%s' and createdon<'%s' ) ''' \
         % (start_time,end_time,start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_row(config.conn_viva,sql)
    print value
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.moblog_msgs_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva,sql)
    print value
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.polls_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva,sql)
    print value
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.comments_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2 and owner_type=11" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva,sql)
    print value
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.comments_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2 and owner_type=12" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva,sql)
    print value
    ...
    db='mozone_tag'
    sql="select count(id) from mozone_tag.dbo.rss_comments_tags with(nolock) where createdon>='%s' and createdon<'%s' and partner_id =2" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva,sql)
    print value
