...
def stat_sub(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='umniah_mt'
    sql="select count(*) from [umniah_mt].dbo.logs with(nolock) where action=10 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    ...
    db='umniah_mt'
    sql="select count(*) from [umniah_mt].dbo.logs with(nolock) where action=11 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_umniah,sql)
    print value
    ...
    db='umniah_mt'
    sql="select count(*) from [umniah_mt].dbo.accounts with(nolock) where is_deleted=0"
    
    """
    sql=r'''
    
    select top 1 NumOfSubscribers
    from [umniah_mt].[dbo].[num_of_subscribers]
    ...
    and [msisdn] not in (
        SELECT distinct [msisdn] 
          from [umniah_mt].[dbo].[system_action_logs] with(nolock)
        where action=3
        and [CreatedOn]<'%s'
    )    
