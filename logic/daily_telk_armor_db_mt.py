...
def stat_mt(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='telkomsel_mt'
    sql="select count(*) from telkomsel_mt.dbo.charge_logs with(nolock) where createdon between '%s' and '%s'" \
         % (start_time,end_time)
    ...
    db='telkomsel_mt'
    sql="select count(*) from telkomsel_mt.dbo.charge_logs with(nolock) where status=1 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_telk_armor,sql)
    print value
