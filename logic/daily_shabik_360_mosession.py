...
def stat_mosession(my_date):
    ...
    exec('from user_id_filter.user_id_filter_shabik_360_'+current_date.replace('-','_')+' import is_valid_user')

    group_key_current_day=datetime.fromtimestamp(my_date).strftime('%Y-%m-%d')

    current_day_for_mosession_refined=my_date
    current_day_for_mosession_refined_ceil=helper_regex.time_ceil_timestamp(current_day_for_mosession_refined)+3600*5
    current_day_for_mosession_refined_floor=helper_regex.time_floor_timestamp(current_day_for_mosession_refined)+3600*5
    ...
    stat_sql_login_ip_daily_by_ip_range_shabik_360=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_shabik_360':is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'},
                                   db_name='raw_data_shabik_360')

    stat_plan.add_stat_sql(stat_sql_login_ip_daily_by_ip_range_shabik_360)
    


    stat_sql_data_usage_total_daily_shabik_360=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   ...
                                   where={'refined_data_usage':is_valid_date_mosession_refined,'only_shabik_360':is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined},
                                   db_name='raw_data_shabik_360')

    stat_plan.add_stat_sql(stat_sql_data_usage_total_daily_shabik_360)

    stat_sql_online_time_daily_shabik_360=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   ...
                                   where={'online_time_refined':is_valid_login_date_mosession_refined,'only_shabik_360':is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined},
                                   db_name='raw_data_shabik_360')

    stat_plan.add_stat_sql(stat_sql_online_time_daily_shabik_360)
