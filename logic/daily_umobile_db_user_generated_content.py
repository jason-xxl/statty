...
def stat_user_generated_content(my_date): # run on 0:00 a.m. , calculate yesterday's data
    ...
    oem_name='Mozat'
    stat_category='user_generated_content_only_umobile'
    ...
    values=user_id_filter_umobile.get_filtered_dict(values)
    ...
    sql="""select count(id) from mozone.dbo.photos with(nolock) where createdon>='%s' and createdon<'%s' 
    	and user_id in (SELECT user_id FROM [mozone_user].[dbo].[ProfileSub] with(nolock) where version_tag ='fast_umobile') """ \
         % (start_time,end_time)
    ...
    sql="""select count(id) from [FEED_DB].[dbo].[ITEM_COMMENT] with(nolock) where [DATE]>='%s' and [DATE]<'%s'
    	and [COMMENT_USER_ID] in (SELECT user_id FROM [mozone_user].[dbo].[ProfileSub] with(nolock) where version_tag ='fast_umobile') """ \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat,sql)
    print value
