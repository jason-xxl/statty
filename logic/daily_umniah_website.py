...
def stat_website(my_date):
    ...
    stat_sql_client_download_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'j2me':r'(/)download/umniah\.jar', \
                                                       'symbian_2':r'(/)download/shabik\-2\.sis', \
                                                       'symbian_3':r'(/)download/shabik\-3\.sisx', \
                                                       'symbian_5':r'(/)download/shabik\-5\.sisx', \
                                                       'blackberry_start':r'(/)download/umniahbb.*?\.jad', \
                                                       'blackberry_finish':r'(/)download/umniahbb.*?\-6\.cod'}, \
    ...
    stat_sql_client_download_phone_brand_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   ...
                                   where={'client_download':r'(GET|POST) /download/shabik'}, \
    ...
    stat_sql_client_download_public_phone_brand_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'phone':r'(.)'}, \
                                   where={'client_download':r'(GET|POST) /download/down'}, \
                                   group_by={'daily':get_current_date,'client_brand':get_phone_brand})
    ...
    # get symbian 2 user count from j2me user

    def is_simbian_2(line):
        return helper_regex.get_supported_platform_from_browser_user_agent(line)=='SymbianOS-2'

    stat_sql_client_download_os_s2_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'download':r'(.)'}, \
                                   where={'j2me':r'(/)download/umniah\.jar',
                                          'is_simbian_2':is_simbian_2}, \
                                   group_by={'daily':get_current_date})
    
    stat_plan.add_stat_sql(stat_sql_client_download_os_s2_daily)
