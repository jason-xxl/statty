...
def stat_invite(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    sql=r'''
    select count(*) from [invitation_umniah].dbo.nativeSms_record with(nolock) where date between '%s' and '%s'
    ''' % (start_time,end_time)
    ...
    sql=r'''
    select count(*) from [umniah_mt].dbo.award_record with(nolock) where createdON between '%s' and '%s'
    ''' % (start_time,end_time)
    ...
    (
        ...
        and [msisdn] in (
            ...
            FROM [umniah_mt].[dbo].[award_record]
        )
    ) a
