...
def stat_mt(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    from billing_stc.dbo.addvalue with(nolock) 
    ...
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,sum(values.values()),table_name='raw_data_shabik_360')
    ...
    print len(values)
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,len(values),table_name='raw_data_shabik_360')
    ...
    select user_id,sum(money) 
    from billing_stc.dbo.payment with(nolock) 
...
if __name__=='__main__':
    ...
    for i in range(config.day_to_update_stat,0,-1):
        ...
        if helper_regex.get_weekday_from_time_stamp(my_date)==7:
            weekly_shabik_360_db_billing.stat_mt(my_date)
