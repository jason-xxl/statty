...
def stat_billing(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='billing_umniah'
    sql="select item_name, sum(money) as amount from billing_umniah.dbo.payment with(nolock) where time_payment between '%s' and '%s' group by item_name order by item_name" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    values=helper_sql_server.fetch_dict(config.conn_umniah,sql)
    ...
    db='billing_umniah'
    sql="select item_name, count(distinct user_id) as amount from billing_umniah.dbo.payment with(nolock) where time_payment between '%s' and '%s' group by item_name order by item_name" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    values=helper_sql_server.fetch_dict(config.conn_umniah,sql)
    ...
    db='billing_umniah'
    sql="select item_name, count(distinct [index]) as amount from billing_umniah.dbo.payment with(nolock) where time_payment between '%s' and '%s' group by item_name order by item_name" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    values=helper_sql_server.fetch_dict(config.conn_umniah,sql)
