...
import helper_regex
import config
import weekly_mozat_db_billing
...
def stat_mt(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='billing'
    sql=r"""
    select sum([billing].dbo.ConvertCurrency(money,currency,'USD'))
    from billing.dbo.addvalue with(nolock)
    where time_addvalue between '%s' and '%s'
    """ % (start_time,end_time)
    
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_float(config.conn_mozat,sql)
    print value
...
if __name__=='__main__':
    ...
    for i in range(config.day_to_update_stat,0,-1):
        ...
        if helper_regex.get_weekday_from_time_stamp(my_date)==7:
            weekly_mozat_db_billing.stat_mt(my_date)
            pass
