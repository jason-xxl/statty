...
def is_target_files(line):
    ...
    if '/360/' in file_name or '360.aspx' in file_name:
        return file_name
    return ''

def is_download_finished (line):
    
    download_url_prefix='http://shabik.com.bh'
...
def stat_website(my_date):
    ...
    stat_plan_reform_206_request.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
                                        my_date,r'\\192.168.0.140\logs_website_viva_bh\ex%(date)s%(hour)s.log', \
                                        timezone_offset_to_sg=config.timezone_offset_viva_bh,date_format='%y%m%d'))

    """

    stat_plan_reform_206_request.add_log_source(r'\\192.168.0.140\logs_website_viva_bh\ex' \
