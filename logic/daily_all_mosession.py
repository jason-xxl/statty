...
def stat_mosession(my_date):
    ...
    stat_sql_login_ip_daily_by_ip_range_stc=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_stc':user_id_filter_stc.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_daily_by_ip_range_stc)
    
    """
    stat_sql_login_ip_hourly_by_ip_range_stc=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_stc':user_id_filter_stc.is_valid_user}, \
                                   group_by={'hourly':get_login_time_date_hour_mosession,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_hourly_by_ip_range_stc)
    """

    stat_sql_data_usage_total_daily_stc=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_sum={'bytes':get_data_usage_mosession_refined}, \
                                   select_sum_top={'top_max_100':{'key':r'\d{2}: (\d+)','value':get_data_usage_mosession_refined,'limit':100}}, \
                                   where={'refined_data_usage':is_valid_date_mosession_refined,'only_stc':user_id_filter_stc.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_data_usage_total_daily_stc)

    stat_sql_online_time_daily_stc=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_distinct_collection={'monet_id':r': (\d+),'}, \
                                   select_average={'per_session':get_online_time_mosession_refined}, \
                                   select_count_exist={'zero_online_time':is_zero_online_time_mosession}, \
                                   select_sum_top={'top_max_100_in_sec':{'key':r'\d{2}: (\d+)', \
                                   ...
                                   select_sum_average={'daily_online':{'key':r'\d{2}: (\d+)','value':get_online_time_mosession_refined},
                                                       'daily_online_distribution':{'key':r'\d{2}: (\d+)', \
                                                                                    'value':get_online_time_mosession_refined,
                                                                                    'sec_group_key':get_online_time_level_mosession}}, \
                                   where={'online_time_refined':is_valid_login_date_mosession_refined,'only_stc':user_id_filter_stc.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_online_time_daily_stc)

    #STC end

    #Viva begin

    stat_sql_login_ip_daily_by_ip_range_viva=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_viva':user_id_filter_viva.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_daily_by_ip_range_viva)

    """
    stat_sql_login_ip_hourly_by_ip_range_viva=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_viva':user_id_filter_viva.is_valid_user}, \
                                   group_by={'hourly':get_login_time_date_hour_mosession,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_hourly_by_ip_range_viva)
    """

    stat_sql_data_usage_total_daily_viva=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_sum={'bytes':get_data_usage_mosession_refined}, \
                                   select_sum_top={'top_max_100':{'key':r'\d{2}: (\d+)','value':get_data_usage_mosession_refined,'limit':100}}, \
                                   where={'refined_data_usage':is_valid_date_mosession_refined,'only_viva':user_id_filter_viva.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_data_usage_total_daily_viva)

    stat_sql_online_time_daily_viva=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_distinct_collection={'monet_id':r': (\d+),'}, \
                                   select_average={'per_session':get_online_time_mosession_refined}, \
                                   select_count_exist={'zero_online_time':is_zero_online_time_mosession}, \
                                   select_sum_top={'top_max_100_in_sec':{'key':r'\d{2}: (\d+)', \
                                   ...
                                   select_sum_average={'daily_online':{'key':r'\d{2}: (\d+)','value':get_online_time_mosession_refined},
                                                       'daily_online_distribution':{'key':r'\d{2}: (\d+)', \
                                                                                    'value':get_online_time_mosession_refined,
                                                                                    'sec_group_key':get_online_time_level_mosession}}, \
                                   where={'online_time_refined':is_valid_login_date_mosession_refined,'only_viva':user_id_filter_viva.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_online_time_daily_viva)

    #Viva end

    #Viva_BH begin

    stat_sql_login_ip_daily_by_ip_range_viva_bh=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_viva_bh':user_id_filter_viva_bh.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_daily_by_ip_range_viva_bh)
    
    """
    stat_sql_login_ip_hourly_by_ip_range_viva_bh=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'login':r'(NORMAL)'}, \
                                   where={'filtered':is_valid_login_date_mosession_refined,'only_viva_bh':user_id_filter_viva_bh.is_valid_user}, \
                                   group_by={'hourly':get_login_time_date_hour_mosession,'by_ip_range':r'(\d+\.\d+\.)\d+\.\d+,'})

    stat_plan.add_stat_sql(stat_sql_login_ip_hourly_by_ip_range_viva_bh)
    """

    stat_sql_data_usage_total_daily_viva_bh=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_sum={'bytes':get_data_usage_mosession_refined}, \
                                   select_sum_top={'top_max_100':{'key':r'\d{2}: (\d+)','value':get_data_usage_mosession_refined,'limit':100}}, \
                                   where={'refined_data_usage':is_valid_date_mosession_refined,'only_viva_bh':user_id_filter_viva_bh.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_data_usage_total_daily_viva_bh)

    stat_sql_online_time_daily_viva_bh=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_distinct_collection={'monet_id':r': (\d+),'}, \
                                   select_average={'per_session':get_online_time_mosession_refined}, \
                                   select_count_exist={'zero_online_time':is_zero_online_time_mosession}, \
                                   select_sum_top={'top_max_100_in_sec':{'key':r'\d{2}: (\d+)', \
                                   ...
                                   select_sum_average={'daily_online':{'key':r'\d{2}: (\d+)','value':get_online_time_mosession_refined},
                                                       'daily_online_distribution':{'key':r'\d{2}: (\d+)', \
                                                                                    'value':get_online_time_mosession_refined,
                                                                                    'sec_group_key':get_online_time_level_mosession}}, \
                                   where={'online_time_refined':is_valid_login_date_mosession_refined,'only_viva_bh':user_id_filter_viva_bh.is_valid_user}, \
                                   group_by={'daily':get_current_date_mosession_refined})

    stat_plan.add_stat_sql(stat_sql_online_time_daily_viva_bh)
    ...
    stat_plan.add_log_source(r'\\192.168.0.79\monet_log\MoSession_'+date_next_day_str) #stc
    stat_plan.add_log_source(r'\\192.168.0.100\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.107\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.108\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.117\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.118\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.75\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.103\logMonet\MoSession_'+date_next_day_str)
    stat_plan.add_log_source(r'\\192.168.0.140\MoNET\bin\Log\MoSession_'+date_next_day_str) #baharin
    stat_plan.add_log_source(r'\\192.168.0.122\monet\MoNET\bin\Log\MoSession_'+date_next_day_str) #kwait
    stat_plan.add_log_source(r'\\192.168.0.185\log_mosession_shabik_360\MoSession_'+date_next_day_str) 
    stat_plan.add_log_source(r'\\192.168.0.196\log_mosession_shabik_360\MoSession_'+date_next_day_str) 
