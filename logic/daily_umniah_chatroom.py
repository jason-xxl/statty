...
def stat_chatroom(my_date):
    ...
    stat_plan.add_log_source(r'\\192.168.1.40\umniah_chatroom_logs\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.40\logs_umniah_chatroom\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.1.40\chatroom-shabik\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
    
    stat_plan.add_log_source(r'\\192.168.0.150\logs_chatroom_umniah\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
