...
def stat_invite(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    sql=r'''
    select count(*) from [invitation_viva].dbo.nativeSms_record with(nolock) where date between '%s' and '%s'
    ''' % (start_time,end_time)
    
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_invitation,sql)
    print value
    ...
    sql=r'''
    select count(*) from viva_mt.dbo.award_record with(nolock) where createdON between '%s' and '%s'
    ''' % (start_time,end_time)
    ...
    (
        ...
        where [CreatedOn] < '%s'
        group by [msisdn]
        having max([id]*100+[action])-max([id]*100) in (2)
        and min([CreatedOn]) >= '%s'
        and [msisdn] in (
        
            SELECT distinct [msisdn] collate database_default 
            FROM [DB86].[viva_mt].[dbo].[award_record]
        )
    ) a
    ''' % (end_time,start_time)
