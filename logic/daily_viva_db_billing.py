...
def stat_mt(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='billing_viva'
    sql="select sum(money) from billing_viva.dbo.addvalue with(nolock) where time_addvalue between '%s' and '%s'" \
         % (start_time,end_time)
    ...
    db='billing_viva'
    sql="select count(distinct user_id) from billing_viva.dbo.addvalue with(nolock) where time_addvalue between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    ...
    db='billing_viva'
    sql="select count(distinct [index]) from billing_viva.dbo.addvalue with(nolock) where time_addvalue between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_billing,sql)
    print value
    ...
    db='billing_viva'
    sql="select sum(money) from billing_viva.dbo.payment with(nolock) where time_payment between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_float(config.conn_viva_billing,sql)
    print value
    ...
    db='billing_viva'
    sql="select count(distinct user_id) from billing_viva.dbo.payment with(nolock) where time_payment between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_billing,sql)
    print value
    ...
    db='billing_viva'
    sql="select count(distinct [index]) from billing_viva.dbo.payment with(nolock) where time_payment between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_billing,sql)
    print value
...
if __name__=='__main__':
    ...
    for i in range(config.day_to_update_stat+15,0,-1):
        ...
        if helper_regex.get_weekday_from_time_stamp(my_date)==7:
            weekly_viva_db_billing.stat_mt(my_date)
