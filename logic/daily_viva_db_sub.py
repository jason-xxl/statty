...
def stat_sub(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='viva_mt'
    sql="select count(*) from viva_mt.dbo.logs with(nolock) where action=2 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    ...
    db='viva_mt'
    sql="select count(*) from viva_mt.dbo.logs with(nolock) where action=6 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_mt,sql)
    print value
    ...
    from [viva_mt].[dbo].[num_of_subscribers]
    where [OfDate] > DATEADD(day,1,'%s')
    order by [OfDate] asc
    
    ''' % (date_today,)

    #sql="select count(*) from viva_mt.dbo.accounts with(nolock) where is_deleted=0"
    ...
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_row(config.conn_viva_mt,sql)
    print value
