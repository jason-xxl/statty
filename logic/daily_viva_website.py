...
def stat_website(my_date):
    ...
    stat_sql_client_download_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'j2me':r'(/)download/(?:shabik|viva).*?\.jar', \
                                                       'symbian_2':r'(/)download/(?:shabik|viva).*?\-2\.sis', \
                                                       'symbian_3':r'(/)download/(?:shabik|viva).*?\-3\.sisx', \
                                                       'symbian_5':r'(/)download/(?:shabik|viva).*?\-5\.sisx', \
                                                       'blackberry_start':r'(/)download/(?:shabik|viva)bb.*?\.jad', \
                                                       'blackberry_finish':r'(/)download/(?:shabik|viva)bb.*?\-5\.cod'}, \
    ...
    stat_sql_client_download_phone_brand_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   ...
                                   where={'client_download':r'(GET|POST) /download/(?:shabik|viva)'}, \
    ...
    stat_plan.add_log_source(r'\\192.168.0.122\w3svc473342829\ex' \
           +datetime.fromtimestamp(my_date).strftime('%y%m%d') \
           +'.log')

    stat_plan.add_log_source(r'\\192.168.0.122\log_viva_web\ex' \
           +datetime.fromtimestamp(my_date).strftime('%y%m%d') \
           +'.log')
