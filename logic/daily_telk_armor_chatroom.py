...
# note: the chatroom node is for Telk_armor only, no need to fileter monet_id
...
def stat_chatroom(my_date):
    ...
    stat_plan.add_log_source(r'\\192.168.1.36\logs_chatroom\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))

    stat_plan.add_log_source(r'\\192.168.100.20\logs_chatroom\service.log.' \
           +datetime.fromtimestamp(my_date).strftime('%Y-%m-%d'))
