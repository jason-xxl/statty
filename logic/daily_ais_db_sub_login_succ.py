...
def stat_login(my_date): # run on 0:00 a.m. , calculate yesterday's data
    ...
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,len(new_user_user_set),table_name='raw_data_ais')

    key='user_new_logined_1_day_unique'
    print len(logined_new_user_user_set)
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,len(logined_new_user_user_set),table_name='raw_data_ais')
