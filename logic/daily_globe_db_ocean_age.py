import helper_sql_server
import helper_mysql
import os
from stat_plan import Stat_plan
from stat_sql import Stat_sql
from datetime import datetime
import time
import helper_regex
import config


def stat_ocean_age(my_date): # run on 0:00 a.m. , calculate yesterday's data

    oem_name='Mozat'
    stat_category='ocean_age_only_globe'
        
    start_time=helper_regex.time_floor(my_date)
    end_time=helper_regex.time_ceil(my_date)
    date_today=start_time.replace(' 00:00:00','')
    
    # oa level 1
    
    key='level_1_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=1
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)


    # oa no money user
    
    key='level_1.1_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=1 and money>0
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)


    
    # oa level 2
    
    key='level_2_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=2
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)
    
    # oa level 3
    
    key='level_3_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=3
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    # oa level 4
    
    key='level_4_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=4
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    # oa level 5
    
    key='level_5_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=5
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    # oa level 6
    
    key='level_6_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=6
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    # oa level 7
    
    key='level_7_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where shipyardlevel=7
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    # oa no money user
    
    key='no_money_user_unique'
    
    db='OceanAge_Globe'
    sql=r'''
    select count(*) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where money=0
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    
    # oa daily player
    
    key='daily_user_unique'

    db='OceanAge_Globe'
    sql=r'''
    select count(distinct monetId) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where [lastLogin]>=DATEADD(day,-1,Getdate())
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)

    
    
    
    # oa monthly player
    
    key='monthly_user_unique'

    db='OceanAge_Globe'
    sql=r'''
    select count(distinct monetId) 
    from OceanAge_Globe.dbo.Fisher with(nolock)
    where [lastLogin]>=DATEADD(day,-30,Getdate())
    and monetId not in (
        select user_id
        from mozone_user.dbo.profile with(nolock) 
        where version_tag='fast_globe'
        and user_name like '%00000%' 
    )
    '''
    
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_mozat_oa,sql)
    print value
    helper_mysql.put_raw_data(oem_name,stat_category,key,date_today,value)


if __name__=='__main__':

    for i in range(config.day_to_update_stat,0,-1):
        my_date=time.time()-3600*24*i
        stat_ocean_age(my_date)

