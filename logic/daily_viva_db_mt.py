...
def stat_mt(my_date): # run on 5:00 a.m. , calculate yesterday's data
    ...
    db='viva_mt'
    sql="select count(*) from viva_mt.dbo.charge_logs with(nolock) where createdon between '%s' and '%s'" \
         % (start_time,end_time)
    ...
    db='viva_mt'
    sql="select count(*) from viva_mt.dbo.charge_logs with(nolock) where status=5 and createdon between '%s' and '%s'" \
         % (start_time,end_time)
    print 'SQL Server:'+sql
    value=helper_sql_server.fetch_scalar_int(config.conn_viva_mt,sql)
    print value
