...
def stat_website(my_date):
    ...
    stat_sql_client_download_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                   select_count_exist={'j2me':r'(/)v/en\.jar', \
                                                       'windows_me':r'(/)v/mozat_Installer_en\.cab'}, \
    ...
    log_files_1=r'\\192.168.0.130\log_morange_com_website\ex' \
           +datetime.fromtimestamp(my_date).strftime('%y%m%d') \
           +'.log'
    
    print 'log pattern: '+log_files_1
