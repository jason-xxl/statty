...
#Fields: date time cs-method cs-uri-stem cs-uri-query c-ip cs-version cs(User-Agent) cs(Cookie) cs(Referer) sc-status sc-bytes time-taken
2012-06-12 13:00:17 GET /360/stc.jad - 84.235.73.239 HTTP/1.1 NokiaC3-00/5.0+(07.20)+Profile/MIDP-2.1+Configuration/CLDC-1.1+Mozilla/5.0+AppleWebKit/420++(KHTML,+like+Gecko)+Safari/420+ ASP.NET_SessionId=1nrr1nkjvoespahyjgg5zuwf;+msisdn=966550646702/eurl.axd/f1db8d162050f84d9d0320cde8bf9e35/;+rand=1500814243 http://i.shabik.net.sa/360.aspx?no=966550646702/eurl.axd/f1db8d162050f84d9d0320cde8bf9e35/ 304 213 78
...
Response code:
1 206 partial content, rely on rand to connect them. 
'''
 
def is_target_files(line):
    '''
    todo: should also check the response status is less than 400
    '''
    if line.find('Python-urllib')>-1:
        return ''
    file_name=helper_regex.extract(line,r'(?:GET|POST) ([^\s]+)').lower()
    d = tool_qlx.get_iis_datetime(line)
    if d and d <= datetime(2012,6,28,9,0,0): # utc0 time
...
#    \== tool_qlx.get_utc_datetime(my_time, tool_qlx.UTC0).date())
    ...
    stat_sql_channel_download_user_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                        select_count_distinct_collection={'session':r'NET_SessionId=([0-9a-z]+)'},\
                                        select_count_exist={'action':r'(.)'},\
                                        where={'filtered':lambda line:True,\
                                               'from_channel':r'(f(?:rm)?=)'}, \
    ...
    stat_plan_detailed.add_stat_sql(stat_sql_channel_download_user_daily)
    ...
    stat_plan_reform_206_request.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
                                        my_date,r'\\192.168.0.103\w3svc1213381794\ex%(date)s%(hour)s.log', \
                                        timezone_offset_to_sg=config.timezone_offset_shabik_360,date_format='%y%m%d'))

    stat_plan_reform_206_request.add_log_source_list(helper_regex.translate_iis_website_hourly_log_path(
                                        my_date,r'\\192.168.0.117\w3svc1213381794\ex%(date)s%(hour)s.log', \
                                        timezone_offset_to_sg=config.timezone_offset_shabik_360,date_format='%y%m%d'))
    """

#    stat_plan_reform_206_request.add_url_sources(helper_regex.translate_iis_website_hourly_log_path(
...
#                                        timezone_offset_to_sg=config.timezone_offset_shabik_360+8,date_format='%y%m%d'))
    ...
    if current_date_object <= date(2012,6,27):
        stat_plan_reform_206_request.add_url_sources(tool_qlx.fill_time_pattern_string(\
    ...
    target_keys=[
        ['Shabik_360','website','filtered_9.file-type-start_daily_msisdn_unique','client-start-file'],
        ['Shabik_360','website','filtered_1.cli-type_daily_msisdn_unique','Android'],
        ['Shabik_360','website','filtered_8.special-file-type_daily_msisdn_unique','bb-start-file'],
        ['Shabik_360','website','filtered_8.special-file-type_daily_msisdn_unique','jme-end-file'],
        ['Shabik_360','website','filtered_1.cli-type_daily_msisdn_unique','Symbian-3'],
        ['Shabik_360','website','filtered_1.cli-type_daily_msisdn_unique','Symbian-5'],
    ]
