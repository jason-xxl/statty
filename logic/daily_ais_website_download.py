...
def is_target_files(line):
    ...
    if '/360/' in file_name or '/comp-test/' in file_name or '360.aspx' in file_name:
        return file_name
    """
    if 'loops' in file_name or 'download/' in file_name:
        return file_name
    return ''

def is_download_finished (line):
    
    download_url_prefix='http://210.193.49.127'
...
def stat_website(my_date):
    ...
    oem_name='Mozat'
    stat_category='website_only_ais'
    
    db_name='raw_data_ais'
    ...
    stat_sql_download_user_daily=Stat_sql(oem_name=oem_name,stat_category=stat_category, \
                                        select_count_distinct_collection={'session':r'sess=([0-9a-z]+)', \
                                                               'msisdn':r'msisdn=(\d+)', \
    ...
    stat_plan_reform_206_request.add_log_source(r'\\192.168.0.130\log_download\ex' \
