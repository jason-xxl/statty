...
function _generate_transposed_table(){
	...
	html='<table id="main_table_temp" style="display:none;">'+html+'</table>';
	//console.log(html);
...
function _get_cell_content(column_name,value_of_key_row,table_id){
	table_id=table_id||'table.main_table';
	var column_names=$(table_id+' thead tr th');
	...
	target_cell=html.split('</td>')[column_index].replace('<td><span class="hidden"></span>','').replace('<td>','');
	//console.log(target_cell);

	//console.log(column_name,value_of_key_row,table_id,target_cell);
	return target_cell;
}
...
function add_highcharts_basic_line_chart(args){

	var args_defaults={
		'tab_name':'test_tab',
		'column_names_pattern':'',
		'marginRight':200,
	...
	args=$.extend(args_defaults,args);

	var tab_name=args['tab_name'];//
	var marginRight=args['marginRight'];//
	var reverse_key_column=args['reverse_key_column'];//
	var reverse_column=args['reverse_column'];//
	var reverse_table=args['reverse_table'];//
	var exclude_rows=args['exclude_rows'];//
	var column_names_pattern=args['column_names_pattern'];//
	...
	var key_column_values_temp=_get_key_column_values(table_id);
	var key_column_values=[];
	for (var i=0; i<key_column_values_temp.length; i++){
		if (exclude_rows.indexOf(key_column_values_temp[i])==-1){
			key_column_values[key_column_values.length]=key_column_values_temp[i];
		}
	}

	for (var i=0;i<key_column_values.length;i++ ){
		key_column_values[i]=_extract_cell_content(key_column_values[i]);
		//console.log(key_column_values[i]);
		...
		var p=/(Created in \[\d+d,\d+d\])/ig;
		if(p.test(key_column_values[i])){
			var l=key_column_values[i].length-9;
			key_column_values[i]=key_column_values[i].substring(l);
			continue;
		}
	}
	...
	column_names=_get_column_names(column_names_pattern,table_id);
	
	if(reverse_column){
		column_names=column_names.reverse();
	}
	...
	for (var i=0;i<column_names.length ;i++ ){
		...
		for (var j=0; j<key_column_values.length; j++){
			...
			var d=_get_number(_get_cell_content(column_names[i],key,table_id));
			if (!d && d!='0' && d!=0){
				console.log(d);
				d=null;
			}
	...
	if(getParameterByName('hide_chart_lines')){
